...
export interface TileInstance {
  ...
  /** claude/shell (agent) only — a hivemind issue this running agent is
   *  associated with, set MANUALLY (from the tile header's "link to task"
   *  button or from the issue peek). Bidirectional + persisted so the link
   *  survives reload: the issue peek lists the tiles linked to it (with live
   *  status), and the tile header shows the issue id as a chip. `issueRoot` is
   *  the `.hivemind` root that owns the id (the issue may live in another
   *  workspace than the tile's frame). Cleared to unlink. */
  issueId?: string;
  issueRoot?: string;
}
