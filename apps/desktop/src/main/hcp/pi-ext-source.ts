 ...
 * `HIVE_HCP_SOCK` + `HCP_TOKEN` + `HIVEMIND_TILE` from env; it no-ops (stays a plain pi session) unless hivemind injected the
 * socket + tile.
 *
 * Two capabilities, both fail-open (never throw out of a handler, never hang pi):
 ...
 * site below says why a fail-closed gate here was worse than no gate.
 *
...
export function piExtSource(): string {
  return `// auto-generated by hivemind — pi lifecycle → HCP bridge + native hive orchestration tools.
