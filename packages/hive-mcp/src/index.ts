  ...
  // ── canvas / agent control (HCP — needs the desktop app running) ──────────
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: { type: "string", description: "claude only — model alias like 'opus' or 'sonnet'; omit for the workspace default." },
        ...
        supervise: { description: "CLAUDE WORKERS ONLY — spawning a 'pi' worker with supervise is an ERROR (pi has no permission system, so there is nothing to broker; a pi worker always runs autonomously). Routes a claude worker's tool-permission prompts to YOU (the spawner) to approve via hive_approve, instead of a human. true / 'parent' brokers the mutating tools (Bash/Edit/Write/WebFetch); 'all' brokers every tool; or pass a comma-string / array of tool names. Omit for normal (human) permissions. Note each brokered call costs you a turn to answer — an 'allow' is remembered for the file-touching tools, but Bash re-asks every time. Fails safe to the human prompt if you don't answer.", type: ["boolean", "string", "array"], items: { type: "string" } },
      },
    },
  },
  {
