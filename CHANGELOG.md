...
- **Tile names no longer fall back to "claude #N · auto" after a session reattach or reboot.** An agent tile's resolved name is claude's live OSC window-title task summary (e.g. "Fix AI Chatbot Service Request subject") — it isn't stored in the session `.jsonl`, so the only durable source is the title itself. The daemon's replay uses `SerializeAddon`, which does **not** serialize the window title, so a reattach never re-fired the title event and the name fell back to the generic spawn label. The **daemon now owns the title as session state**: it captures each session's OSC title, persists it in the session snapshot, and re-emits it ahead of the replay on every attach (live reattach *and* reboot-restore) so the client's xterm re-fires `onTitleChange`. One authoritative source — the earlier renderer-side localStorage cache (v1.12.4) is removed. A user rename still takes precedence.
...
- **Layers panel visual polish.** Workspace icons are now color-keyed folder glyphs that open/close with the group (a repo workspace *is* a folder), with worktrees shown as a branch glyph and remotes as a server glyph — one consistent family, no boxed chip. Nested worktrees and tiles now hang from a faint vertical guide rail so the hierarchy is legible at a glance.
...
[Unreleased]: https://github.com/dip497/hivemind/compare/v1.19.2...HEAD
[1.19.2]: https://github.com/dip497/hivemind/releases/tag/v1.19.2
